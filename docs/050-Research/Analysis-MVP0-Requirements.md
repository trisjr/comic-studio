...
| **Q-3** | Chốt **vendor** image + VLM cho MVP0 | ⚠️ **Đã thu hẹp**: [ADR-007](../030-Specs/Architecture/ADR-007-VLM-Provider-For-QA-Select.md) `Q4` đặt việc chốt **vendor VLM** ở **gate cuối MVP0** — đầu vào của quyết định là chính số đo MVP0 ⇒ ⛔ **không chặn MVP0**. Vendor **ảnh** thì repo đã ngầm chốt **Gemini 3 Pro Image**: mọi con số chi phí (`$0.134`, `~$12`, `$12,06`) tính từ nó. Còn lại: **xác nhận** lựa chọn vận hành, ⛔ không phải quyết định mở | **Founder + Architect** |
