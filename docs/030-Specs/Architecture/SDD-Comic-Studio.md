...
| **`T-26`** **Danh mục kiểu bubble** (speech / thought / shout / whisper…) và **hình dạng dữ liệu của SFX / narration box / caption** | [ADR-013](./ADR-013-Typeset-Layer-Separate-From-Art.md) · `UC-07` bước 6 và `AF-6` · findings/architect §7 `G9` | **PM hỏi Founder** → Architect ghi vào lô DB Schema / API | **Trước khi viết DDL của typeset layer**. ⛔ Không được suy ra danh mục — nguồn ⛔ chưa định nghĩa ở đâu. ⭐ Phiếu quyết định đã soạn: [`Decision-Sheet-T26-Bubble-Kind-And-SFX.md`](./Decision-Sheet-T26-Bubble-Kind-And-SFX.md) |
| ⭐ **`T-30`** *(bổ sung `2026-09-04`)* **Hướng đọc trang** — trái→phải (webtoon / phương Tây) hay phải→trái (manga Nhật). ⚠️ ⛔ **Không** phải hàng i18n `T-28`: đó là ngôn ngữ giao diện, đây là **hướng dẫn mắt người đọc qua panel** | `DB-Entity-Comic-IR.md` (`panel_index` chỉ khai **thứ tự**, ⛔ không khai **hướng**) | **PM hỏi Founder** — ⛔ quyết định sản phẩm, ⛔ không suy ra được từ code | ⚠️ **Trigger đã biết, ⛔ không phải giả định**: art style đã chốt là **manhwa/webtoon** nhưng dữ liệu MVP0 là **12 trang rời khổ A5**; [findings/architect §8](../../010-Planning/pm-runs/2026-08-23-danh-gia-y-tuong-comic-studio/findings/architect.md) ghi *"Webtoon là dải dọc liên tục, không có 'trang', không có gutter ngang […] ⛔ Không thể lấy layout thiết kế cho trang A5 rồi tự động ra webtoon"* ⇒ đóng **trước** khi chốt hình dạng `page_layout` (`T-31`) |
...
**Nhóm F — ⭐ LỖ PHÁT HIỆN SAU PHASE 2** *(bổ sung `2026-09-04`)* — kỹ thuật, **Architect đóng ở lô kế tiếp**

> ⚠️ **Vì sao là nhóm riêng, ⛔ không nhét vào [§9.2](#92-tbd-còn-trong-phase-2--đã-chuyển-cho-lô-sau)**: §9.2 là *"còn **TRONG** Phase 2"*, mà Phase 2 đã có [`handoff.md`](../../010-Planning/pm-runs/2026-08-28-phase-2-architecture-design-comic-studio/handoff.md) và lô DB Schema đã chạy xong. Thêm hàng vào §9.2 là giao việc cho một lô ⛔ **không còn tồn tại** — hàng sẽ nằm im ⛔ không ai nhận.

| `TBD` | Thuộc tài liệu nào | Ai đóng | Khi nào / điều kiện mở khoá |
|---|---|---|---|
| ⭐ **`T-31`** **Hình dạng JSON của `page_layout`** — `LT-1` đòi *"đúng cùng một **hình dạng JSON**"* nhưng ⛔ **không nơi nào trong repo định nghĩa hình dạng đó**. `INV-5` chỉ ràng buộc **giá trị** (mọi toạ độ trong `[0,1]`), ⛔ không ràng buộc **cấu trúc**. Ba lỗ là **một câu hỏi**, ⛔ không phải ba: ① **tier/row** — dữ liệu MVP0 dùng khoá `rows` nhưng khoá này ⛔ không xuất hiện ở schema hay Glossary ② **`x`/`w` của panel trong cùng một row** — ⛔ không tồn tại, dù **7 row** trong `panel-script-ch1/ch2` có ≥2 panel ⇒ chia ngang hiện là **ngầm định trong đầu người viết**, ⛔ không phải dữ liệu ③ **gutter** — ⛔ không có trường nào; dữ liệu thật có `sum(h) = 1.00` ở **mọi** trang ⇒ các row **tiếp giáp**, gutter **bằng 0** | `DB-Entity-Comic-IR.md` (`LT-1`, `INV-5`, `INV-6`) · [ADR-012](./ADR-012-Comic-IR-Spec-As-Primary-Data.md) điều 5–6 · `mvp0/panel-script-ch1.yaml` | **Architect** | **Trước khi viết DDL của `comic.page`**. ⚠️ Phụ thuộc `T-30`: hướng đọc quyết định trang-rời hay dải-dọc, mà hai thứ đó ⛔ **không cùng một hình dạng JSON**. ⛔ Không được đóng bằng cách chép khoá `rows` của MVP0 — dữ liệu đó là **nguyên liệu viết tay**, ⛔ không phải đặc tả |

⇒ **31 hàng** ở §9.1, trong đó **7 hàng chờ luật sư** (`T-18`…`T-24`) và **2 hàng bổ sung sau Phase 2** (`T-30`, `T-31`).
