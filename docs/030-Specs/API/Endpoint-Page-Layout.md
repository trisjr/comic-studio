...
| `T-PL-BUDGET-UNIT` | **Đơn vị của `text_budget`** (ký tự hay từ) và **hàm tính từ diện tích** — hàng đã đăng ký ở [`DB-Entity-Comic-IR.md`](../Schema/DB-Entity-Comic-IR.md). ⭐ **Hàm tính PHỤ THUỘC metric của font render** ⇒ phải đóng **SAU** `TBD-FONT` của [`ADR-013`](../Architecture/ADR-013-Typeset-Layer-Separate-From-Art.md) §*Thứ tự đóng hai `TBD`*. ⚠️ Đóng ngược thứ tự ⇒ hàm phải calibrate lại ⇒ `text_budget` đổi. ⭐ **Đây là CÙNG LẬP LUẬN với `T1` của `D-33`, ⛔ KHÔNG phải cùng trigger** — `T1` kích hoạt bởi **diện tích panel đổi**, còn ở đây diện tích ⛔ không đổi, thứ đổi là **hàm**. ⇒ ⛔ **Không đường code nào tự bắt được**; hệ quả tuỳ thời điểm (chưa có dòng nào PASS gate #2 ⇒ calibration sai bị nướng vào spec; đã có ⇒ phải reset về `OPEN`) — đọc đủ ở [`ADR-013`](../Architecture/ADR-013-Typeset-Layer-Separate-From-Art.md) §*Thứ tự đóng hai `TBD`* *(phần **đơn vị** thì ⛔ không phụ thuộc font — chốt độc lập được)* | **BA + Architect** | Trước gate `M2-3`. ⚠️ **Cận dưới đề xuất: sau `G1-e`** — ⛔ **CHƯA được chủ hàng phê duyệt**, ghi bởi lô đồng bộ `2026-08-30`; **BA + Architect** xác nhận hoặc bác |
