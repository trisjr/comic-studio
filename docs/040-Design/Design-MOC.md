...
| `DS-005` | ⭐ [Typography](./Design-System/Typography.md) | **HAI hệ font, token KHÔNG chung namespace**: font UI là CSS variable ở `apps/web`; font render vào ảnh là **tham số config của `apps/api`**, phải **đơn trị**, ⛔ không fallback stack. Giá trị font render là `TBD` do `ADR-013` sở hữu — file này chỉ ghi **ràng buộc** |
