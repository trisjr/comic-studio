...
| [Analysis-MVP0-Requirements](./050-Research/Analysis-MVP0-Requirements.md) | Phân tích yêu cầu **MVP0** — sẵn sàng thực thi, ma trận Story × tiêu chí `G1`, 8 phát hiện lệch tài liệu |
...
| **9** | 🆕 ⭐ **51 anchor trong-file gãy + 8 link gãy trên toàn `docs/`** — phát hiện ngày `2026-08-30` bằng [`scripts/check-doc-anchors.py`](../scripts/check-doc-anchors.py) *(chạy `python3 scripts/check-doc-anchors.py docs`, ⛔ không trích lại số)*. ⚠️ **`51` là CẬN DƯỚI, ⛔ không phải tổng**: script chỉ kiểm anchor **trong-file**; anchor **liên-file** (`file.md#anchor`) ⛔ chưa được kiểm. ⭐ **Nguyên nhân gốc của 13/51**: anchor có **em-dash `—`** viết dạng `#hệ-1-—-font-ui`, nhưng `github-slugger` **loại bỏ** `—` (nằm trong dải `U+2000–U+206F`) ⇒ slug thật là `#hệ-1--font-ui` (**hai** gạch nối). ⚠️ Đây là **mở rộng của nợ Phase 2 #8** (*"1 anchor gãy cứng + ~29 gãy mềm"*) — nay đã **đo được toàn cây**, ⛔ không còn là ước lượng. Về 8 link gãy: **7 gãy thật + 1 dương tính giả** *(ở `pm-runs/…/dong-bo-srs-nfr-voi-adr/findings/architect.md` — trích dẫn minh hoạ, script ⛔ chưa gỡ được)*. 5/7 nằm ở `pm-runs/` (run-state, ⛔ không phải deliverable); 3 còn lại ở `Manuals-MOC` / `Deployment-MOC` trỏ tới tài liệu chưa viết — ⚠️ **chúng là bullet trần, ⛔ KHÔNG mang nhãn `TBD`/`planned`** ⇒ ⛔ không đọc được là placeholder có chủ ý, cần chủ tầng xác nhận | Thấp — điều hướng, ⛔ không sai nội dung | — |
